...
def test_downstream_intent_resolution():
...
def test_upstream_intent_resolution():
    resolver = RuleBasedIntentResolver()
...
def test_neighbors_intent_resolution():
    resolver = RuleBasedIntentResolver()
...
def test_unsupported_questions_fail_safely():
    resolver = RuleBasedIntentResolver()
    with assert_raises(ValueError, match="Could not determine a supported query intent"):
        resolver.resolve("What is the cost of pump 101?")

    with assert_raises(ValueError, match="Could not determine a supported query intent"):
        resolver.resolve("Can you operate valve V-101?")

    with assert_raises(ValueError, match="Query question cannot be empty"):
        resolver.resolve("   ")


def test_missing_entity_fails_safely():
    resolver = RuleBasedIntentResolver()
    with assert_raises(ValueError, match="Could not identify a valid entity reference"):
        resolver.resolve("What is downstream?")


def test_ambiguous_conflicting_intents_fail_safely():
    resolver = RuleBasedIntentResolver()
...
def test_paths_between_intent_resolution():
    resolver = RuleBasedIntentResolver()
    ...
    with assert_raises(ValueError, match="Paths-between query requires both source and target"):
